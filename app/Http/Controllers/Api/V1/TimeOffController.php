    ...
    public function store(CreateTimeOffRequest $request): JsonResponse
    {
        try {
            ...
            try {
                ...
                $endDate = \Carbon\Carbon::parse($request->end_date)->format('M d, Y');
                $dateRange = $startDate === $endDate ? $startDate : "{$startDate} - {$endDate}";

                TableNotification::create([
                    'type' => TableNotification::TYPE_TIME_OFF_REQUEST,
                    'title' => 'New Time Off Request',
                    'message' => "{$employeeName} has requested {$request->type} time off for {$dateRange}.",
                    'order_number' => null,
